...
Current release: page **2.13.0**, engine **2.3.0**, protocol **1**.
...
An **Examples** dropdown sits beside the Make heading. Choosing one of the five
loads it into the board and says what went in. It does not press the disc: you
press Make when you are ready, the way you would with anything you typed. An
example replaces whatever is on the board, so it asks first if there is
anything there.
...
and a rehearsal calendar for a community theatre that does not exist. Each row
carries a size word for the picture it makes: four are Small and Tom Sawyer is
Medium, because an automatic size is floored at 256 pixels so the artwork stays
visible and only Tom Sawyer climbs past it.
