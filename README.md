...
Pasting or dropping a PuttyPNG while Made! is up asks **Are you sure?** first,
because opening it replaces the picture you made. Download or copy that one
first if you want to keep it.

## Editing a disc title

After making a disc, press **Edit Title** in the row under the picture on a
desktop, or **Edit** on a phone. The editor holds one field, one sentence and one
...
There is one of each kind: *The Adventures of Tom Sawyer*, which is public
domain writing, and a rehearsal calendar for a community theatre that does not
exist. Each disc says which one it holds across its top. Both make a picture
under 512 pixels, so neither needs a warning.
...
standard depth is one byte per pixel. Made!'s window then draws the disc at
that size rather than at the capacity band it falls in.
